 ...
 AccelStepper engineX (AccelStepper :: DRIVER, stepX, dirX); // with step pin & dir pin
 AccelStepper engineY (AccelStepper :: DRIVER, stepY, dirY);
 AccelStepper engineY (AccelStepper :: DRIVER, stepZ, dirZ);
 ...
 void pilotMotors (long mapX, long mapY) {
   ...
   if (mapY <= tresholdDown) {
	 ...
	 speedZ = map (mapY, maxSpeed, tresholdUp, maxSpeed, minSpeed);
     moveY = true;
   } else {
     // y stands still
     speedY = 0;
